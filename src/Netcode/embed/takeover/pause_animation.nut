...
function Initialize()
{
	...
	item_table.save_state <- ["Save State"]; // takeover
	item_table.load_state <- ["Load State"]; // takeover (继续放录像)
	item_table.takeover_state <- ["Takeover P1"]; // takeover (接管P1)
	item_table.takeover_p2_state <- ["Takeover P2"]; // takeover (接管P2)
